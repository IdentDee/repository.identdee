...
site = AdultSite('heavy-r', '[COLOR hotpink]Heavy-R[/COLOR]', 'https://www.heavy-r.com/', BASE_URL + '/images/logo.png', 'heavy-r')
...
def Main():
    order = utils.addon.getSetting('heavyrsortorder') if utils.addon.getSetting('heavyrsortorder') else 'recent/'
    ...
    site.add_dir('[COLOR hotpink]Search[/COLOR]', BASE_URL + '/index.php', 'Search', site.img_search)
    List(BASE_URL + '/videos/' + order)
...
def List(url):
    ...
    re_npnr = r'(\d+)\/">Next<\/a>'
    re_lpnr = r'(\d+)\/">Last<\/a>'
    utils.next_page(site, 'heavy-r.List', html, re_npurl, re_npnr=re_npnr, re_lpnr=re_lpnr)
    utils.eod()
