...
site = AdultSite('katitube', "[COLOR hotpink]KatiTube[/COLOR]", 'https://katitube.com/', 'https://katitube.com/templates/default/images/logo_header.png', 'katitube')
...
def Main():
    order = utils.addon.getSetting('katitubesortorder') if utils.addon.getSetting('katitubesortorder') else 'most-recent/'
    ...
    site.add_dir('[COLOR hotpink]Categories[/COLOR]', site.url + 'channels/', 'ShowCategories', site.img_cat)
    List(site.url, True)
    utils.eod()

@site.register()
def List(url, ordered: bool = False):
    order = ""
    if ordered:
    	order = utils.addon.getSetting('katitubesortorder') if utils.addon.getSetting('katitubesortorder') else 'most-recent/'
    html = utils.getHtml(url + order, site.url)
    ...
    utils.videos_list(site, 'katitube.Play', html, delimiter, re_videopage, re_name, re_img, re_duration=re_duration, contextm='katitube.Related')

    re_npurl = r'<a href=\'([^\']+)\' class="next">Next'
    re_npnr = r'<a href=\'page(\d+)\.html\' class="next">Next'
    current_url = url
    ...
    if match:
        ...
        site.add_dir('Next Page {}'.format(np), npurl, 'katitube.List')
    utils.eod()

@site.register()
def ShowCategories(url):
    html = utils.getHtml(url)
    pattern = re.compile(r'<div class="title-thumb-channel">.*?<a href="([^"]+)">([^<]+)', re.DOTALL | re.IGNORECASE)
    for (categoryUrl, categoryName) in re.findall(pattern, html):
...
@site.register()
def Related(url):
    contexturl = (utils.addon_sys + "?mode=" + str('katitube.List') + "&url=" + urllib_parse.quote_plus(url))
...
def Sortorder(url):
    ...
    if order:
        utils.addon.setSetting('katitubesortorder', sort_orders[order])
        Main()
